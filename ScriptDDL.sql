...
/*  Created On : 27-Sep-2019 04:35:28 p.m. 				*/
/*  DBMS       : Oracle 					        	*/
/* ---------------------------------------------------- */
...
(
	K_ID NUMBER(8) NOT NULL,    -- ID de la categoria
	N_NOMBRE VARCHAR2(30) NOT NULL,    -- Nombre de la categoria
	V_IVA NUMBER(5,2) NOT NULL    -- Porcentaje de iva que se debe pagar en esa categoria de productos
)
;
...
(
	...
	N_NOMBRE VARCHAR2(20) NOT NULL,    -- Nombre de la clasificacion del re. de ventas
...
(
	F_NUMERO_ID VARCHAR2(40) NOT NULL,    -- Numero de identificacion del cliente
	F_TIPO_ID CHAR(1) NOT NULL,    -- Tipo de ID 
	T_TELEFONO NUMBER(12) NOT NULL,    -- telefono del cliente
	E_CORREO VARCHAR2(50) NOT NULL,    -- Correo del cliente
	F_ID_REP_VENTAS VARCHAR2(40) NOT NULL,    -- ID del prepresentante de ventas que lo vinculo
...
(
	...
	V_PRECIO NUMBER(6) NOT NULL,    -- Precio del producto en esa region
	...
	F_CODIGO_POSTAL VARCHAR2(7) NOT NULL    -- Recibe la llave foranea del codigo postal de la region
)
;
...
(
	...
	F_NUMERO_ID VARCHAR2(40) NOT NULL,    -- Recibe la llave foranea de la tabla cliente
...
(
	K_NUMERO_ID VARCHAR2(40) NOT NULL,    -- Numero de identificacio de la persona
	K_TIPO_ID CHAR(1) NOT NULL,    -- Tipo de documento de la persona
	N_NOMBRE VARCHAR2(25) NOT NULL,    -- Nombre de la persona
	N_APELLIDO VARCHAR2(25) NOT NULL,    -- Apellido de la persona
	A_DIRECCION VARCHAR2(50) NOT NULL,    -- Direccion de residencia de la persona
	C_CIUDAD VARCHAR2(25) NOT NULL    -- Ciudad de la persona
)
;
...
(
	...
	N_NOMBRE_PRODUCTO VARCHAR2(30) NOT NULL    -- Nombre del producto
)
;

CREATE TABLE  producto_subcategoria
(
	K_ID NUMBER(7) NOT NULL,    -- Identificador de la tabla de rompimiento
	F_CODIGO_PRODUCTO NUMBER(8) NOT NULL,    -- Llave foranea con el identificador del producto
	F_ID NUMBER(8) NOT NULL    -- Lllave foranea de la subcategoria
)
;

CREATE TABLE  region
(
	K_CODIGO_POSTAL VARCHAR2(7) NOT NULL,    -- Codigo postal de la region
	N_NOMBRE VARCHAR2(30) NOT NULL    -- nombre de la regional
)
;

CREATE TABLE  rep_ventas
(
	F_NUMERO_ID VARCHAR2(40) NOT NULL,    -- Identificador de la persona que es representante de ventas
	F_TIPO_ID CHAR(1) NOT NULL,    -- Tipo de ID de la persona
	E_CORREO VARCHAR2(50) NOT NULL,    -- Correo del representante de ventas
	...
	F_ID_CLASIFICACION NUMBER(2) NOT NULL,    -- Recible llave foranea de la tabla clasificacion
	F_ID_REP_CAPATADOR VARCHAR2(40) NULL,    -- Recible llave foranea de otro representante de ventas que lo ha captado
	F_TIPO_ID_REP_CAPATADOR CHAR(1) NULL,    -- Recible llave foranea de otro representante de ventas que lo ha captado
	F_CODIGO_POSTAL VARCHAR2(7) NOT NULL    -- Recible llave foranea de a region a la que pertenece el representante de ventas
)
;
...
(
	...
	N_NOMBRE VARCHAR2(30) NOT NULL,    -- Nombre de la subcategoria
	FK_K_ID NUMBER(8) NOT NULL    -- Llave foranea con 
)
;
...
COMMENT ON TABLE  categoria IS 'Categoria de los productos'
;

COMMENT ON COLUMN  categoria.K_ID IS 'ID de la categoria'
;

COMMENT ON COLUMN  categoria.N_NOMBRE IS 'Nombre de la categoria'
;

COMMENT ON COLUMN  categoria.V_IVA IS 'Porcentaje de iva que se debe pagar en esa categoria de productos'
;
...
COMMENT ON TABLE  producto_subcategoria IS 'Tabla de rompimiento entre producto y subcategoria'
;

COMMENT ON COLUMN  producto_subcategoria.K_ID IS 'Identificador de la tabla de rompimiento'
;

COMMENT ON COLUMN  producto_subcategoria.F_CODIGO_PRODUCTO IS 'Llave foranea con el identificador del producto'
;

COMMENT ON COLUMN  producto_subcategoria.F_ID IS 'Lllave foranea de la subcategoria'
;
...
COMMENT ON COLUMN  rep_ventas.F_ID_CLASIFICACION IS 'Recible llave foranea de la tabla clasificacion'
;
...
COMMENT ON TABLE  subcategoria IS 'Contiene las subcategorias y sus caracteristicas'
;

COMMENT ON COLUMN  subcategoria.K_ID IS 'Llave primaria de la subcategoria'
;

COMMENT ON COLUMN  subcategoria.N_NOMBRE IS 'Nombre de la subcategoria'
;

COMMENT ON COLUMN  subcategoria.FK_K_ID IS 'Llave foranea con '
;
...
ALTER TABLE  producto_subcategoria 
 ADD CONSTRAINT CK_ID_PRODUCTO_SUBCATEGORIA CHECK (K_ID > 0)
;
...
ALTER TABLE  subcategoria 
 ADD CONSTRAINT CK_ID_SUBCATEGORIA CHECK (K_ID > 0)
;
...
ALTER TABLE  subcategoria 
 ADD CONSTRAINT FK_subcategoria_categoria
	FOREIGN KEY (FK_K_ID) REFERENCES  categoria (K_ID)
;
...
create view V_PRODUCTO as select i.K_ID,i.V_PRECIO, p.N_NOMBRE_PRODUCTO, c.N_NOMBRE, c.V_IVA
from inventario i, producto p,producto_subcategoria ps,subcategoria s,categoria c
where i.F_CODIGO_PRODUCTO=p.K_CODIGO_PRODUCTO and ps.F_CODIGO_PRODUCTO=p.K_CODIGO_PRODUCTO and ps.F_ID=s.K_ID and s.FK_K_ID = c.K_ID;
